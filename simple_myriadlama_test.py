...
def run_simple_test(method="avg", num_samples=250):
    ...
    subset_data = [dataset.ds[i]
                   for i in range(min(num_samples, len(dataset.ds)))]
...
def main():
    ...
    print("=" * 60)
    print(
        f"📈 AVG Method Accuracy: {avg_accuracy:.4f} ({sum([1 for r in avg_results if calculate_simple_accuracy([r]) > 0])}/{len(avg_results)})")
    print(
        f"📈 MAX Method Accuracy: {max_accuracy:.4f} ({sum([1 for r in max_results if calculate_simple_accuracy([r]) > 0])}/{len(max_results)})")
