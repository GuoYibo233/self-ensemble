...
def test_quick_self_ensemble():
    ...
    for i, item in enumerate(test_data, 1):
        ...
        first_words = [resp.split()[:2]
                       for resp in all_responses if resp.strip()]
        first_words_str = [' '.join(words) for words in first_words if words]
        if first_words_str:
            most_common = Counter(first_words_str).most_common(1)[0]
            print(
                f"  🏆 Most Common Start: {most_common[0]} (appears {most_common[1]} times)")

        # 平均长度
        avg_length = np.mean([len(resp.split())
                             for resp in all_responses if resp.strip()])
