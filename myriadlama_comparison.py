...
def create_subset_dataset(model_name, subset_size=1000, seed=42):
    ...
    if subset_size > full_size:
        print(f"⚠️ Requested subset size ({subset_size}) is larger than dataset ({full_size})")
...
def run_self_ensemble(model_name, subset_data, dataset, method, max_new_tokens=20):
    ...
    for item in tqdm(subset_data, desc=f"Processing {method}"):
        ...
        if method == "avg":
            # 平均集成方法
            prediction = ensemble_avg_generation(model, tokenizer, prompts, max_new_tokens)
        elif method == "max":
            # 最大集成方法
            prediction = ensemble_max_generation(model, tokenizer, prompts, max_new_tokens)
        else:
...
def ensemble_avg_generation(model, tokenizer, prompts, max_new_tokens=20):
    ...
    # 编码所有prompts
    inputs = tokenizer(prompts, padding=True, truncation=True, return_tensors="pt", padding_side="left")
    ...
    with torch.no_grad():
        ...
        for _ in range(max_new_tokens):
            ...
            new_input = torch.cat([input_ids[0:1], next_token.unsqueeze(0)], dim=1)
            new_attention = torch.ones_like(new_input)
            
            # 获取下一个token的logits
            next_outputs = model(input_ids=new_input, attention_mask=new_attention)
...
def evaluate_accuracy(results):
    ...
    for result in results:
        ...
        gold_answers.append([[ans.lower().split() for ans in result["answers"]]])
...
def main():
    ...
    print("\\n🎯 RESULTS SUMMARY")
    print("=" * 60)
    print(f"📈 AVG Method Accuracy: {avg_accuracy:.4f} ({sum(avg_matches)}/{len(avg_matches)})")
    print(f"📈 MAX Method Accuracy: {max_accuracy:.4f} ({sum(max_matches)}/{len(max_matches)})")
