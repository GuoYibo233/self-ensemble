...
def normalize_chunk(chunk):
    ...
    sample_lemmas = []
    answer_lemmas = []
    for answers, greedy_predict, sample_predicts in tqdm(
        zip(chunk["answers"], chunk["greedy_predict"], chunk["sample_predicts"]),
        total=len(chunk),
    ):
...
if __name__ == "__main__":
    ...
    if args.write_confidence:
        assert os.path.exists(
            dump_path
        ), f"Confidence scores file {dump_path} does not exist. Please run the script with --rewrite to create it."
        df = pd.read_feather(dump_path)
        if (
            "confidence" in df.columns
            and "greedy_lemma" in df.columns
            and "sample_lemmas" in df.columns
            and "answer_lemmas" in df.columns
        ):
            print(
    ...
    for uuids, answers, all_paraphrases in tqdm(dataloader):
        for paraphrases in all_paraphrases:
            ...
            sample_predicts = [
                ...
                for samples in multinormal_samples
            ]
            for (
                uuid,
                prompt,
                ...
                uuids, prompts, answers, paraphrases, greedy_predicts, sample_predicts
            ):
                item = {
                    "uuid": uuid,
