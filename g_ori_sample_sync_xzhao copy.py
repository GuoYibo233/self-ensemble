...
def ensemble_generation(prompt_sets, integration_method="max", weights=None):
    ...
    # For confidence-based weighting, we'll compute weights from first token
    first_token_confidences = None
    ...
    for step in range(max_new_tokens):
        ...
        # Compute first token confidence weights on the first step
        if step == 0 and integration_method == "fast_confidence":
            # Get softmax probabilities for each prompt set
            probs_set = logits_set.softmax(dim=-1)  # [num_prompts, batch_size, vocab_size]
            # Get max probability for each prompt set (the confidence)
            max_probs = probs_set.max(dim=-1).values  # [num_prompts, batch_size]
            # Use inverse as weight (higher confidence = lower weight, as specified)
            first_token_confidences = 1.0 / max_probs  # [num_prompts, batch_size]
            # Normalize weights to sum to 1 across prompts for each batch item
            first_token_confidences = first_token_confidences / first_token_confidences.sum(dim=0, keepdim=True)
            # Store for return
            computed_weights = first_token_confidences[:, 0].cpu().tolist()
        ...
        elif integration_method == "fast_confidence":
            # Use first token confidence as weights for all subsequent tokens
            if first_token_confidences is None:
                raise ValueError("First token confidences not computed.")
            # Apply confidence weights to logits
            weighted_logits = (logits_set * first_token_confidences.unsqueeze(-1)).sum(dim=0)
    ...
    # Stop at newline to get only the first line
    new_generated_texts = [gen.split('\n')[0].strip() for gen in generated_texts]
    return new_generated_texts, computed_weights
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", type=str, default="per_prompt", choices=["origin", "per_prompt", "max", "avg", "weighted_avg", "weighted_max", "fast_confidence"],
    ...
    if args.method == "per_prompt":
        ...
        for uuid, answer, sampled_paraphrases in tqdm(all_samples, desc="Generating"):
            all_prompts = []
            confidences = [] if args.method.startswith("weighted_") and args.method != "fast_confidence" else None
            
            # For ensemble generation, treat each paraphrase as a separate prompt
            for para in sampled_paraphrases:
                if args.method.startswith("weighted_") and args.method != "fast_confidence":
            ...
            result = ensemble_generation(all_prompts, integration_method=args.method, weights=[confidences] if confidences else None)
            if isinstance(result, tuple):
                generations, computed_weights = result
            else:
                generations = result
                computed_weights = None
            predictions = [gen.strip().split('\n')[0] for gen in generations]
            ...
            if computed_weights is not None:
                items["confidence_weights"] = [computed_weights]
            df = pd.concat([df, pd.DataFrame(items)], ignore_index=True)
